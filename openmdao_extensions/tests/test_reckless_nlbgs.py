...
class TestRecklessNLBGS(unittest.TestCase):
    def setUp(self):
        self.prob = Problem()
        ...
        model.add_subsystem(
            "pz", IndepVarComp("z", np.array([5.0, 2.0])), promotes=["z"]
        )

        model.add_subsystem(
            "d1", ContrivedSellarDis1(), promotes=["x", "z", "y1", "y2"]
        )
    ...
    def test_bad_size(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            self.prob.run_model()
        self.assertEqual(
            str(context.exception), "Convergence rtols bad size : should be 2, found 1."
        )
