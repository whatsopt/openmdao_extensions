...
class TestSmtDoeDriver(unittest.TestCase):
    ...
    def test_smt_lhs_doe_driver(self):
        n=10
        self.assert_case_generation(n, SmtDOEDriver(sampling_method_name='LHS', 
                                                    n_cases=n, 
    ...
    def test_smt_ff_doe_driver(self):
        n=11
        self.assert_case_generation(n, SmtDOEDriver(sampling_method_name='FullFactorial', 
                                                    n_cases=n))
    @unittest.skipIf(SMT_NOT_INSTALLED, 'SMT library is not installed')
    def test_smt_rand_doe_driver(self):
        n=12
        self.assert_case_generation(n, SmtDOEDriver(sampling_method_name='Random', 
                                                    n_cases=n))

    @unittest.skipIf(SMT_NOT_INSTALLED, 'SMT library is not installed')
    def test_deprecated(self):
        msg = "'SmtDoeDriver' is deprecated; " \
